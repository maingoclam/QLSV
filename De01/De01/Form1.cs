...
namespace De01
{
    public partial class Form1 : Form
    {
        ...
        {
            string connectionString = "Data Source=NGOCLAM_2104\\MAILAM;Initial Catalog=QuanLySV;Integrated Security=True"; // Thay thế bằng chuỗi kết nối của bạn
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            {
                string maSV = row.Cells["MaSV"].Value.ToString().ToLower(); // Mã sinh viên
                string tenSV = row.Cells["HotenSV"].Value.ToString().ToLower(); // Tên sinh viên

                if (maSV.Contains(button8) || tenSV.Contains(button8))
                {
                    row.Selected = true;
                    lvSinhVien.CurrentCell = row.Cells["HotenSV"]; // Di chuyển tới cột tên sinh viên
                    row.Cells["HotenSV"].Selected = true;
                    row.Cells["MaSV"].Selected = true; // Chọn cả cột mã sinh viên
                    row.Cells["NgaySinh"].Selected = true; // Chọn cột ngày tháng năm sinh
                }
                else
                {
                    row.Selected = false;
